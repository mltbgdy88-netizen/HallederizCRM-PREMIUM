...
import { useAyarlarReferenceData } from "@/features/ayarlar/hooks/use-ayarlar-reference-data";

function HubIcon({ icon }: { icon: AhbCard["icon"] }) {
  const props = {
    width: 22,
    height: 22,
    viewBox: "0 0 24 24",
...
export function AyarlarHubPage() {
  const {
    data: { title: AHB_TITLE, subtitle: AHB_SUBTITLE, cards: AHB_CARDS }
  } = useAyarlarReferenceData();
      ...
      <div className="ahb-grid">
        {AHB_CARDS.map((card) => {
          ...
          return card.href.startsWith("/") ? (
            ...
            <article key={card.id} className="ahb-card ahb-card--static">
              {inner}
            </article>
          );
        })}
