...
const DEMO_ALERTS: AlertCard[] = [
  { key: "overdue", title: "Vadesi GeÃ§en Tahsilatlar", countLabel: "8 kayÄ±t", amountLabel: "â‚º2.810.000", action: "Listeyi GÃ¶r", href: "/tahsilatlar", tone: "danger", icon: "bell-ring" },
  { key: "approvals", title: "Onay Bekleyen Ä°ÅŸlemler", countLabel: "5 iÅŸlem", action: "OnaylarÄ± AÃ§", href: "/onaylar", tone: "gold", icon: "clipboard-check" },
  { key: "stock", title: "Stokta Azalan ÃœrÃ¼nler", countLabel: "12 Ã¼rÃ¼n", action: "StoklarÄ± GÃ¶r", href: "/stok", tone: "cream", icon: "package-minus" },
  { key: "delivery", title: "BugÃ¼n Teslim Edilecek", countLabel: "7 sipariÅŸ", action: "TeslimatlarÄ± GÃ¶r", href: "/teslimatlar", tone: "success", icon: "truck" }
];

const DEMO_TASKS: TaskRow[] = [
  { id: "t1", time: "09:00", text: "Vadesi geÃ§en 3 tahsilat iÃ§in mÃ¼ÅŸteri ile gÃ¶rÃ¼ÅŸ", type: "Tahsilat", priority: "YÃ¼ksek" },
  { id: "t2", time: "10:30", text: "ABC GÄ±da - teklif revizesi hazÄ±rla", type: "Teklif", priority: "Orta" },
  { id: "t3", time: "11:00", text: "4 sipariÅŸin sevkiyat planlamasÄ±nÄ± yap", type: "SipariÅŸ", priority: "YÃ¼ksek" },
  { id: "t4", time: "14:00", text: "Stokta azalan Ã¼rÃ¼nler iÃ§in tedarik planÄ± oluÅŸtur", type: "Stok", priority: "Orta" }
];

const DEMO_FLOW: FlowRow[] = [
  { key: "orders", title: "Yeni SipariÅŸler", subtitle: "BugÃ¼n", count: "12", href: "/siparisler", icon: "shopping-cart" },
  { key: "ship", title: "Sevkiyat Bekleyen", subtitle: "Toplam", count: "7", href: "/teslimatlar", icon: "truck" },
  { key: "pay", title: "Tahsilat Bekleyen", subtitle: "BugÃ¼n vadesi gelen", count: "5", href: "/tahsilatlar", icon: "circle-dollar-sign" },
  { key: "return", title: "Ä°ade Talebi", subtitle: "Onay bekleyen", count: "2", href: "/iadeler", icon: "rotate-ccw" },
  { key: "offer", title: "Teklif Bekleyen", subtitle: "YanÄ±t bekleyen", count: "6", href: "/teklifler", icon: "file-text" }
];

const DEMO_RECENT: RecentRow[] = [
  { date: "21.05.2026 10:42", type: "Tahsilat", record: "Tahsilat #TH-2026-0187", customer: "ABC GÄ±da San. Ltd.", amount: "â‚º125.000", status: "TamamlandÄ±" },
  { date: "21.05.2026 10:28", type: "SipariÅŸ", record: "SipariÅŸ #SP-2026-0156", customer: "Vizyon Market", amount: "â‚º85.500", status: "HazÄ±rlanÄ±yor" },
  { date: "21.05.2026 09:55", type: "Teklif", record: "Teklif #TK-2026-0091", customer: "Delta YapÄ±", amount: "â‚º210.000", status: "GÃ¶nderildi" },
  { date: "21.05.2026 09:12", type: "Stok", record: "Stok GiriÅŸi #ST-4412", customer: "â€”", amount: "â€”", status: "KayÄ±tlÄ±" }
];

const QUICK_LINKS: Array<{ href: string; title: string; sub: string; icon: LucideIconName }> = [
  { href: "/siparisler/yeni", title: "Yeni SipariÅŸ", sub: "OluÅŸtur", icon: "plus-square" },
  { href: "/tahsilatlar/yeni", title: "Tahsilat Ekle", sub: "Tahsilat kaydÄ±", icon: "circle-dollar-sign" },
  { href: "/teklifler/yeni", title: "Teklif HazÄ±rla", sub: "Yeni teklif", icon: "file-text" },
  { href: "/cariler/yeni", title: "MÃ¼ÅŸteri Ekle", sub: "Yeni cari kaydÄ±", icon: "user-plus" },
  { href: "/stok", title: "Stok GiriÅŸi", sub: "ÃœrÃ¼n giriÅŸi", icon: "package-plus" },
  { href: "/gorevler", title: "GÃ¶rev OluÅŸtur", sub: "Yeni gÃ¶rev", icon: "clipboard-plus" },
  { href: "/hizli-islem", title: "TÃ¼m Ä°ÅŸlemler", sub: "KÄ±sayollar", icon: "grid-3x3" }
];
...
function taskPriorityLabel(p: Task["priority"]): TaskRow["priority"] {
  if (p === "critical" || p === "high") return "YÃ¼ksek";
...
function taskTypeLabel(task: Task): string {
  ...
  if (task.entityType === "order") return "SipariÅŸ";
...
function buildAlerts(snapshot: DashboardHomeSnapshot, isDemo: boolean): AlertCard[] {
  ...
  return [
    { key: "overdue", title: "Vadesi GeÃ§en Tahsilatlar", countLabel: `${overdue} kayÄ±t`, action: "Listeyi GÃ¶r", href: "/tahsilatlar", tone: "danger", icon: "bell-ring" },
    { key: "approvals", title: "Onay Bekleyen Ä°ÅŸlemler", countLabel: `${approvals} iÅŸlem`, action: "OnaylarÄ± AÃ§", href: "/onaylar", tone: "gold", icon: "clipboard-check" },
    { key: "stock", title: "Stokta Azalan ÃœrÃ¼nler", countLabel: `${stock} Ã¼rÃ¼n`, action: "StoklarÄ± GÃ¶r", href: "/stok", tone: "cream", icon: "package-minus" },
    { key: "delivery", title: "BugÃ¼n Teslim Edilecek", countLabel: `${deliveries} sipariÅŸ`, action: "TeslimatlarÄ± GÃ¶r", href: "/teslimatlar", tone: "success", icon: "truck" }
  ];
}
...
function buildFlow(snapshot: DashboardHomeSnapshot, isDemo: boolean): FlowRow[] {
  ...
  return [
    { key: "orders", title: "Yeni SipariÅŸler", subtitle: "BugÃ¼n", count: snapshot.cardValues.orders ?? "0", href: "/siparisler", icon: FLOW_ICONS.orders },
    ...
    { key: "pay", title: "Tahsilat Bekleyen", subtitle: "BugÃ¼n vadesi gelen", count: snapshot.cardValues.collections ?? "0", href: "/tahsilatlar", icon: FLOW_ICONS.pay },
    { key: "return", title: "Ä°ade Talebi", subtitle: "Onay bekleyen", count: snapshot.cardValues.returns ?? "0", href: "/iadeler", icon: FLOW_ICONS.return },
    { key: "offer", title: "Teklif Bekleyen", subtitle: "YanÄ±t bekleyen", count: snapshot.cardValues.offers ?? "0", href: "/teklifler", icon: FLOW_ICONS.offer }
  ];
}
...
function buildContextItems(alerts: AlertCard[], flowRows: FlowRow[], tasks: TaskRow[]): ContextItem[] {
  ...
  return [
    overdue
      ? {
          title: overdue.title,
          detail: `${overdue.countLabel}${overdue.amountLabel ? ` Â· ${overdue.amountLabel}` : ""}`,
          href: overdue.href,
          tone: "danger",
          icon: overdue.icon
        }
    ...
    firstTask
      ? {
          title: "SÄ±radaki gÃ¶rev",
          detail: `${firstTask.time} Â· ${firstTask.text}`,
          href: "/gorevler",
          tone: firstTask.priority === "YÃ¼ksek" ? "danger" : firstTask.priority === "Orta" ? "gold" : "neutral",
          icon: "clipboard-list"
        }
      : null,
    paymentFlow
      ? {
          ...
          detail: `${paymentFlow.count} kayÄ±t Â· ${paymentFlow.subtitle}`,
...
function DashboardContextPanel({ items, isDemo }: { items: ContextItem[]; isDemo: boolean }) {
  return (
    <article className="hz-dash-card hz-dashboard-context" aria-label="Ana sayfa baÄŸlam paneli">
      <header className="hz-dash-card__head">
        <h2>BugÃ¼nÃ¼n BaÄŸlamÄ±</h2>
      ...
      <div className="hz-dashboard-context__body">
        <div className="hz-dashboard-context__lead">
          ...
          <div>
            <p className="hz-dashboard-context__lead-title">Ã–ncelik sÄ±rasÄ±</p>
            <p className="hz-dashboard-context__lead-copy">
              Ã–nce tahsilat ve onay bekleyen iÅŸleri kapatÄ±n; sonra sevkiyat ve stok uyarÄ±larÄ±nÄ± gÃ¼ncelleyin.
            </p>
          </div>
        </div>
        ...
        <p className="hz-dashboard-context__mode">
          {isDemo ? "Demo veriyle Ã§alÄ±ÅŸÄ±yor. GerÃ§ek kayÄ±t iÃ§in canlÄ± API ve onay akÄ±ÅŸÄ± gerekir." : "CanlÄ± veri baÄŸlÄ±. Kritik iÅŸlemler onay ve audit akÄ±ÅŸÄ±ndan geÃ§er."}
        </p>
      </div>
    </article>
  );
    ...
    <div className="hz-dashboard-command">
      <div className="hz-dashboard-command__grid">
        <div className="hz-dashboard-command__main" style={{ gridTemplateRows: mainGridRows }}>
          ...
          <section className="hz-dash-card hz-dash-card--alerts" data-row="alerts" aria-label="Acil durumlar ve uyarÄ±lar">
                ...
                <article key={card.key} className={`hz-alert-tile hz-alert-tile--${card.tone}`}>
                  ...
                  <div className="hz-alert-tile__copy">
                    ...
                    <p className="hz-alert-tile__value">
                      {card.countLabel}
                      {card.amountLabel ? <span className="hz-alert-tile__meta"> Â· {card.amountLabel}</span> : null}
                    </p>
          ...
          >
            ...
            <section className="hz-dash-card hz-dash-card--tasks">
              <header className="hz-dash-card__head">
                <h2>BugÃ¼nkÃ¼ GÃ¶revlerim ({loading ? "â€¦" : taskCount})</h2>
              ...
              <div className="hz-dash-card__body hz-dash-card__body--flush">
                {tasks.length === 0 ? (
                  <p className="hz-dash-empty">BugÃ¼n iÃ§in gÃ¶rev bulunmuyor.</p>
                ) : (
                  <ul className="hz-task-list">
                    {tasks.map((row) => (
                      <li key={row.id} className="hz-task-row">
                        <input type="checkbox" className="hz-task-row__check" aria-label={`GÃ¶rev: ${row.text}`} readOnly />
                        ...
                        <span
                          className={`hz-badge hz-badge--priority-${
                            row.priority === "YÃ¼ksek" ? "high" : row.priority === "DÃ¼ÅŸÃ¼k" ? "low" : "medium"
                          }`}
                        >
                          {row.priority}
                        </span>
          ...
          <section className="hz-dash-card hz-dash-card--recent">
            ...
            <div className="hz-dash-card__body hz-dash-card__body--flush">
              {recentRows.length === 0 ? (
                <p className="hz-dash-empty">Son iÅŸlem kaydÄ± gÃ¶rÃ¼ntÃ¼lenemiyor.</p>
              ) : (
          ...
          {visiblePanels.quick ? (
          <section className="hz-dash-card hz-dash-card--quick" aria-label="HÄ±zlÄ± iÅŸlemler">
