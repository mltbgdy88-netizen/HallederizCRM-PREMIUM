...
export const DEFAULT_READINESS = [
  ...
  { key: "audit", label: "Denetim izi", ready: false, hint: "Zaman akışı kaydı" },
  { key: "export", label: "Dışa aktar", ready: false, hint: "PDF/Excel hazırlık durumu" }
] as const;
...
export const OFFLINE_API_ROWS: InventoryScopeRow[] = [
  row({
    ...
    readinessChips: ["Sağlık kontrolü", "Yeniden dene"],
...
export const DEMO_MODE_ROWS: InventoryScopeRow[] = [
  ...
  row({
    ...
    status: "ready",
    sourceRoute: "/onaylar",
    readinessChips: ["Onay zinciri", "Denetim izi"],
...
export const LIVE_EMPTY_ROWS: InventoryScopeRow[] = [
  row({
    ...
    readinessChips: ["HTTP 200", "Kiracı kapsamı"],
  ...
  row({
    ...
    status: "ready",
    sourceRoute: "/live-empty",
    readinessChips: ["Sahte veri yok", "Boş durum"],
    description: "Örnek satır veya tamamlanma durumu üretilmez."
  }),
  row({
    id: "create-path",
...
export const PRINT_EXPORT_ROWS: InventoryScopeRow[] = [
  ...
  row({
    ...
    status: "needs-api",
    sourceRoute: "print-export",
    readinessChips: ["exportPdfStatus", "devre dışı"],
    description: "PDF üretimi API hazır olunca; sahte indirme yok."
  }),
  row({
    id: "excel-export",
    ...
    status: "needs-api",
    sourceRoute: "print-export",
    readinessChips: ["exportExcelStatus", "devre dışı"],
    description: "Excel üretimi API hazır olunca; sahte dosya yok."
  })
];
...
export const OFFLINE_API_CONFIG = buildConfig({
  ...
  dataPage: "offline-api-reference",
  title: "Offline API",
  subtitle: "Offline API ekranını gerçek API sonuçları uydurmadan, operasyon ve hazırlık durumu sözleşmesi görünürlüğüyle yönetin.",
  icon: "clock",
  role: "alert",
...
export const DEMO_MODE_CONFIG = buildConfig({
  ...
  dataPage: "demo-mode-reference",
  title: "Önizleme modu",
  subtitle: "Önizleme modu ekranını gerçek API sonuçları uydurmadan, operasyon ve hazırlık durumu sözleşmesi görünürlüğüyle yönetin.",
  icon: "shield",
  mainBadge: {
...
export const LIVE_EMPTY_CONFIG = buildConfig({
  ...
  dataPage: "live-empty-reference",
  title: "Canlı veri boş",
  subtitle: "Canlı veri boş ekranını gerçek API sonuçları uydurmadan, hazırlık durumu sözleşmesi görünürlüğüyle yönetin.",
  icon: "package-minus",
...
export const MOBILE_DRAWER_CONFIG = buildConfig({
  ...
  dataPage: "mobile-drawer-reference",
  title: "Mobile drawer davranış katmanı",
  subtitle: "Mobil menü katmanını gerçek API sonuçları uydurmadan, hazırlık durumu sözleşmesi görünürlüğüyle yönetin.",
  icon: "grid-3x3",
...
export const PRINT_EXPORT_CONFIG = buildConfig({
  ...
  dataPage: "print-export-reference",
  ...
  icon: "file-text",
  alertCopy: "Dışa aktarma ve yazdırma işlemleri API hazır olana kadar devre dışı kalır; sahte dosya veya indirme üretilmez.",
  rows: PRINT_EXPORT_ROWS
});

export const UNAUTHORIZED_ROWS: InventoryScopeRow[] = [
  row({
    id: "page-deny",
...
export const UNAUTHORIZED_CONFIG = buildConfig({
  prefix: "hz-unauthorized-center",
  dataPage: "unauthorized-reference",
  title: "Yetkisiz erişim",
  subtitle: "Erişim reddi ekranını gerçek API sonuçları uydurmadan, hazırlık durumu sözleşmesi görünürlüğüyle yönetin.",
  icon: "shield",
  role: "alert",
  mainBadge: { text: "Erişim reddedildi", tone: "danger" },
  alertCopy: "Bu sayfaya erişim yetkiniz yok. Sessiz yönlendirme veya sahte veri gösterimi yapılmaz.",
  rows: UNAUTHORIZED_ROWS
});
