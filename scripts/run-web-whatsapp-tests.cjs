/**
 * Runs apps/web WhatsApp unit tests (requires apps/web "type": "module" for Node ESM).
 */
...
const testDir = join(webDir, "src", "features", "whatsapp", "__tests__");
const testFiles = readdirSync(testDir)
  .filter((file) => file.endsWith(".test.ts"))
  .sort()
  .map((file) => join("src", "features", "whatsapp", "__tests__", file));

if (testFiles.length === 0) {
  console.error("No WhatsApp test files found.");
  process.exit(1);
}
