/**
 * Runs apps/web Local AI unit tests (requires apps/web "type": "module" for Node ESM).
 */
...
const testDir = join(webDir, "src", "features", "ai", "__tests__");
const testFiles = readdirSync(testDir)
  .filter((file) => file.endsWith(".test.ts"))
  .sort()
  .map((file) => join("src", "features", "ai", "__tests__", file));

if (testFiles.length === 0) {
  console.error("No AI test files found.");
  process.exit(1);
}
