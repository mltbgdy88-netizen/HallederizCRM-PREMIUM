/**
 * Runs apps/web ERP unit tests (requires apps/web "type": "module" for Node ESM).
 */
...
const testDir = join(webDir, "src", "features", "erp", "__tests__");
const testFiles = readdirSync(testDir)
  .filter((file) => file.endsWith(".test.ts"))
  .sort()
  .map((file) => join("src", "features", "erp", "__tests__", file));

if (testFiles.length === 0) {
  console.error("No ERP test files found.");
  process.exit(1);
}
